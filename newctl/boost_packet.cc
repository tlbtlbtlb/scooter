...
packet_rd *
packet_conn_rd_pkt(serial_packet_conn *it)
{
  packet_rd *rd = it->rd_pkt();
  if (rd) {
    return rd;
  } else {
    return new packet_rd("", 0);
  }
}
...
BOOST_PYTHON_MODULE(boost_packet)
{
  ...
  class_<serial_packet_conn>("serial_packet_conn", init<tty_serial *>())
    .def("rd_pkt", &packet_conn_rd_pkt, return_value_policy<reference_existing_object>())
    .def("wr_pkt", &serial_packet_conn::wr_pkt)
    ;
